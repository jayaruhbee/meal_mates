...
class All_meals(APIView):
    def get (self, request):
        ...
        meals = list(set(all_meals))
        serialized_meals = MealSerializer(meals,many = True).data
        return Response(serialized_meals,
                        status = status.HTTP_200_OK)

class All_meals_on_plan(APIView):
    ...
    def get(self, request, meal_plan_id):
        ...
        meal_plan = get_object_or_404(
            Meal_plan, user=request.user, id=meal_plan_id)
...
class Meal_manager(APIView):
    ...
    def get(self, request, meal_plan_id, meal_id):
        ...
        meal = get_object_or_404(
            Day, meal_plan=meal_plan_id, daily_meal=meal_id)
    ...
    def put(self, request, meal_plan_id, day_id, meal_id):
        ...
        a_daily_meal = get_object_or_404(
            Day, meal_plan=meal_plan_id, id=day_id)
    ...
    def post(self, request, meal_plan_id, meal_id):
        ...
        daily_meal_instance = Day.objects.create(
            meal_plan=meal_plan, daily_meal=a_meal)
...
class Meals_manager(APIView):
    ...
    def post(self, request):
        ...
        if request.method == "POST" and request.FILES:
            ...
            if image:
                image_url = f"{settings.MEDIA_URL}{default_storage.save('meal_images/' + image.name, image)}"
            else:
                image_url = None
            # image_url = request.POST.get("image")
            meal = Meal(title=title, category=category, instructions=instructions,
                        ingredients=ingredients, notes=notes, image=image, image_url=image_url)
            meal.full_clean()
            meal.save()
            return Response(MealSerializer(meal).data,
                            status=status.HTTP_201_CREATED)
            ...
            notes = request.POST["notes"]
            meal = Meal(title=title, category=category, instructions=instructions,
                        ingredients=ingredients, notes=notes, image=None, image_url=None)
            meal.full_clean
            meal.save()
            return Response(MealSerializer(meal).data,
                            status=status.HTTP_201_CREATED)
